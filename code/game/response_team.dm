...
/proc/trigger_armed_response_team(var/force = 0, var/reason = "")
	...
	if(!prob(send_team_chance))
		command_announcement.Announce("С территории Комплекса поступил запрос на отправку одной из Мобильно Оперативных Групп. К сожалению, в настоящий момент её отправка является невозможной.", "Оповещение [GLOB.using_map.boss_name]")
		can_call_ert = 0 // Only one call per round, ladies.
		return

	command_announcement.Announce("С территории Комплекса поступил запрос на отправку одной из Мобильно Оперативных Групп. Она будет подготовлена и отправлена в кратчайшие сроки.", "Оповещение [GLOB.using_map.boss_name]")
